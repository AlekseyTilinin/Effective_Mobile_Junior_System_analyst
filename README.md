...
![12345](https://github.com/user-attachments/assets/9e9e1d23-56be-4328-ae90-03d7017da129)

### Что нужно сделать

- Написать пример REST API запроса, который будет вызываться при переходе пользователя на данный экран.
- Привести пример ответа этого REST API в соответствии с макетом.
- Формат — **JSON**.
...
### Описание
Заказчик хочет, чтобы в мобильное приложение интернет-магазина **«Петрушка Зеленая»** начали приходить PUSH-уведомления. Они могут быть разными:  
— о том, что заказ слишком долго лежит без действий в корзине;  
— об отмене заказа;  
— рекламные рассылки и прочее.  

То есть нужен универсальный функционал отправки пушей, а конкретные сценарии будут расширяться со временем.

### Что нужно сделать

Построить верхнеуровневую архитектурную схему того, как должна работать отправка PUSH-уведомлений в данном приложении.  
Можно представить в виде блок-схемы.  

Считаем, что на бэкенде используется **микросервисная архитектура**.  
В данном задании рекомендуется изучить в интернете архитектуры подобных решений.
